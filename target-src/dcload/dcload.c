...
void draw_progress(unsigned int current, unsigned int total)
{
    ...
    draw_string(0, 120, "(", 0xffff);
    draw_string(12, 120, current_string, 0xffff);
    draw_string(108, 120, "/", 0xffff);
    draw_string(120, 120, total_string, 0xffff);
    draw_string(216, 120, ")", 0xffff);
}
...
void disp_info(void)
{
    ...
    setup_video(0,0);
    draw_string(0, 24, NAME, 0xffff);
    draw_string(0, 48, mac_string, 0xffff);
    for(c = 0; c < 4; c++)
      uchar_to_string(ip[3-c], &ip_string[c*3]);
    draw_string(0, 72, ip_string, 0xffff);
    booted = 1;
}
...
/*    scif_init(115200); */
    ...
    if (bb_detect())
	error_bb("NO BROADBAND ADAPTER DETECTED!");

    if (bb_init()<0)
	error_bb("ERROR INITIALIZING BROADBAND ADAPTER!");

    for(start = 0; start < 6; start++)
      uchar_to_string(rtl.mac[start], mac_string + start*3);
